...
class ArtemisJmsClientMessagingTest {
    ...
    static boolean redHatRegistryAuthenticated() {
        try {
            if (SystemUtils.IS_OS_LINUX) {
                // Try docker credentials file
                Path path = Paths.get(System.getProperty("user.home"), ".docker", "config.json");
                File credentialsFile = path.toFile();
                if (credentialsFile.exists()) {
                    String credentials = new String(Files.readAllBytes(path));
                    return credentials.contains("registry.redhat.io");
                }

                // Fallback to podman credentials file
                String xdgRuntimeDir = System.getenv("XDG_RUNTIME_DIR");
                if (xdgRuntimeDir != null) {
                    path = Paths.get(xdgRuntimeDir, "containers", "auth.json");
                    credentialsFile = path.toFile();
                    if (credentialsFile.exists()) {
                        String credentials = new String(Files.readAllBytes(path));
                        return credentials.contains("registry.redhat.io");
                    }
